...
if [ $nx -gt 1 ] ; then
    ...
    sed -i 's/west/stitch_west/g' constant/auxRegion/polyMesh/boundary
    ...
    while [ $i -lt $nx ] ; do
        if [ $((2 * $i)) -le $nx ] ; then
            ...
            sed -i 's/west/stitch_west/g' \
                constant/auxRegionAll/polyMesh/boundary
            sed -i 's/east/stitch_east/g' constant/"${rg}"/polyMesh/boundary
...
if [ $ny -gt 1 ] ; then
    ...
    ## Name stitching region
    sed -i 's/south/stitch_south/g' constant/auxRegion/polyMesh/boundary
    ...
    while [ $j -lt $ny ] ; do
        if [ $((2 * $j)) -le $ny ] ; then
            mkdir -p constant/auxRegionAll
            cp -r constant/"${rg}"/polyMesh constant/auxRegionAll
            sed -i 's/south/stitch_south/g' \
                constant/auxRegionAll/polyMesh/boundary
            sed -i 's/north/stitch_north/g' constant/"${rg}"/polyMesh/boundary
...
if [ $nz -gt 1 ] ; then
    ...
    ## Name stitching region
    sed -i 's/south/stitch_bottom/g' constant/auxRegion/polyMesh/boundary
    ...
    while [ $k -lt $nz ] ; do
        if [ $((2 * $k)) -le $nz ] ; then
            mkdir -p constant/auxRegionAll
            cp -r constant/"${rg}"/polyMesh constant/auxRegionAll
            sed -i 's/south/stitch_bottom/g' \
                constant/auxRegionAll/polyMesh/boundary
            sed -i 's/north/stitch_top/g' constant/"${rg}"/polyMesh/boundary
